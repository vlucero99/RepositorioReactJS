const prendas = [
    {id: 101, imagen: <img src={process.env.PUBLIC_URL + '/img/Remera_musculosa_beige.jpg'} alt="Remera Musculosa Beige" />, nombre: "Músculosa Beige", precio: 3500, descripción: "Remera musculosa color beige de algodón. Viene en talle S al XL.", stock: 20, categoria: "Remeras" },
...
export const getPrendaById = (prendaId) => {
    return new Promise ((resolve) => {
        setTimeout (() => {
            resolve (prendas.find (prod => prod.id === prendaId))
        }, 500)
    })
}
...
export const getPrendasByCategory = (prendaCategory) => {
    return new Promise ((resolve) => {
        setTimeout (() => {
            resolve (prendas.filter(prod => prod.categoria === prendaCategory)) // Usa filter en lugar de find para obtener un array
        }, 500)
    })
}
